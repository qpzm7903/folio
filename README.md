...
最新版本: **v0.25.1** — 重构 PATCH: 版式样式去重。
长期规划 **21/23 完成** (L20 鸿蒙 6.0 适配 / L21 鸿蒙服务卡片 已核心可用, L22 设计系统 2.0 已真机验收, L23 标签管理已发布)。

最近版本:

- **v0.25.1** — 重构 PATCH (行为等价): 9 版式重复的眉注/斜体落款/强调墨色样式
  收敛为三个共享 helper, 一处改全局生效, display_layouts.dart 瘦身 24 行。
- **v0.25.0** — 屏保版式补齐 (L22 收官): 新增 竖 (竖排逐字, 立轴线 + 「金」印) /
  引 (巨型引号 pull-quote) / 条 (通宽纸带, 照片背景仍纸色) / 织 (罗马数字 +
  按标点分句发丝线) 4 款版式, 屏保循环从精选 5 款扩为设计系统全 9 款,
  顺序对照设计源 LAYOUTS。
- **v0.24.x** — 批量导入增强 (分句/去重/勾选保留, 用户需求"导入文本批量建句");
  重构 PATCH: 共享 `XJKSelectCheck` 勾选组件 + 导入行按压反馈与读屏语义。
- **v0.23.x** — 标签管理内联模式 (L23, 「✎ 管理」pill + 删除归「未分类」+
  14-agent 多维审查后发布); 重构 PATCH: 写入链路健壮化 (落盘失败回滚 +
  9 个调用点失败反馈)。
