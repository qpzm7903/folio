...
最新版本: **v0.29.0** — 功能 MINOR: 小组件来源标签 + 每日一句。
长期规划 **21/23 完成** (L20 鸿蒙 6.0 适配 / L21 鸿蒙服务卡片 已核心可用, L22 设计系统 2.0 已真机验收, L23 标签管理已发布)。

最近版本:

- **v0.29.x** — 小组件来源标签: 设置「来源标签」后桌面小组件/鸿蒙卡片
  只轮播该标签下的句子 (标签被删自动回退整库); 频率上限 12h→24h,
  1440 分钟即设计源的「每日一句」。
...
- **v0.24.x 及更早** — 批量导入增强 (分句/去重/勾选保留) / 标签管理内联模式
  (L23) + 写入链路健壮化 / 金库批量操作 (多选+批量取出) / 小组件配色 3→6 /
  三栏导航对齐设计系统 / 鸿蒙卡片视觉改版 / 设计系统 2.0 (六主题+5版式) /
  鸿蒙服务卡片 / 鸿蒙 6.0 适配 / 桌面小组件自动刷新 / drift 持久化 /
